...
function PlanDetails(props: PlanDetailsProps) {
  ...
  //количество вложений
  const [planFilesCount, setPlanFilesCount] = useState<number>(0);

  const fetchPlanFilesCount = async () => {
    const count = await Scripts.getFilesCountPlan(props.data.id);
    setPlanFilesCount(count);
  };
      ...
      {isLoading ? (
        ...
        <div className="plan-details">
          <TabsWrapperNew>
            ...
            <TabItem code={"filesPlan"} name={`Вложения на план (${planFilesCount})`}>
              <FilesTab
                getDataHandler={() => Scripts.getFilesPlan(props.data.id)}
              />
            </TabItem>
          </TabsWrapperNew>
        </div>
